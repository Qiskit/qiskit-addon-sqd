...
def recover_configurations_bcast(
    bitstring_matrix: np.ndarray,
    probabilities: Sequence[float] | np.ndarray,
    avg_occupancies: tuple[np.ndarray, np.ndarray],
    num_elec_a: int,
    ...
    bitstring_matrix_corrected = _bipartite_bitstring_correcting_bscast(
        bitstring_matrix,
        occs_array,
        num_elec_a,
        num_elec_b,
    ...
    for bs_corrected, freq in zip(bitstring_matrix_corrected, probabilities):
        bs_str = "".join("1" if bit else "0" for bit in bs_corrected)
        corrected_dict[bs_str] += freq
...
def _p_flip_0_to_1_bcast(
    ratio_exp: float, occs: np.ndarray, eps: float = 0.01
    ...
    Args:
        ratio_exp: The ratio of 1's expected in the set of bits
        occs: The vector of occupancies for all bits, based estimated ground
            state found at the end of each configuration recovery iteration.
        eps: A value for controlling how aggressively to flip bits
    ...
    """
    if ratio_exp == 1.0:
        return np.ones(len(occs))

    slope = (1 - eps) / (1 - ratio_exp)
    intercept = 1 - slope

    return (occs < ratio_exp) * occs * eps / ratio_exp + (occs >= ratio_exp) * (
        slope * occs + intercept
    )
...
def _p_flip_1_to_0_bcast(
    ratio_exp: float, occs: float, eps: float = 0.01
) -> float:  # pragma: no cover
    """Calculate the probability of flipping a bit from 1 to 0.
    ...
    return (occs < ratio_exp) * (1.0 + occs * (-(1.0 - eps) / ratio_exp)) + (occs >= ratio_exp) * (
        occs * (-eps / (1 - ratio_exp)) + eps / (1 - ratio_exp)
    )
...
def _bipartite_bitstring_correcting_bscast(
    bitstring_matrix: np.ndarray,
    avg_occupancies: np.ndarray,
    hamming_right: int,
    hamming_left: int,
    ...
    Args:
        bitstring_matrix: A 2D array of ``bool`` representations of bit values. Each row is a bitstring
        avg_occupancies: A 1D array containing the mean occupancy of each orbital.
        hamming_right: The target hamming weight used for the right half of the bitstring
        hamming_left: The target hamming weight used for the left half of the bitstring
        rng: A random number generator
    ...
    P_1_to_0_left = _p_flip_1_to_0_bcast(
        hamming_left / partition_size, avg_occupancies[:partition_size], 0.01
    )

    P_0_to_1_right = _p_flip_0_to_1_bcast(
        hamming_right / partition_size, avg_occupancies[partition_size:], 0.01
    )
    P_1_to_0_right = _p_flip_1_to_0_bcast(
        hamming_right / partition_size, avg_occupancies[partition_size:], 0.01
    )

    # Get the probability of flipping each bit, separated into LEFT and RIGHT subsystems,
    # based on the avg occupancy of each bit and the target hamming weight
    probs_left = (1 - bitstring_matrix_left) * P_0_to_1_left + bitstring_matrix_left * P_1_to_0_left
    probs_right = (
        1 - bitstring_matrix_right
    ) * P_0_to_1_right + bitstring_matrix_right * P_1_to_0_right

    # Normalize
    probs_left = np.absolute(probs_left)
    probs_right = np.absolute(probs_right)
    probs_left = probs_left.T
    probs_right = probs_right.T
    probs_left = (probs_left / np.sum(probs_left, axis=0)).T
    probs_right = (probs_right / np.sum(probs_right, axis=0)).T

    ######################## Handle LEFT bits ########################

    # Get difference between # of 1s and expected # of 1s in LEFT bits
    n_left = np.sum(bitstring_matrix_left, axis=1)
    n_diff_left = n_left - hamming_left

    n_right = np.sum(bitstring_matrix_right, axis=1)
    n_diff_right = n_right - hamming_right

    for i in range(bitstring_matrix.shape[0]):
        # Too many electrons in LEFT bits
        if n_diff_left[i] > 0:
            indices_occupied = np.where(bitstring_matrix_left[i])[0]
            # Get the probabilities that each 1 should be flipped to 0
            p_choice = probs_left[i, bitstring_matrix_left[i]] / np.sum(
                probs_left[i, bitstring_matrix_left[i]]
            )
            # Correct the hamming by probabilistically flipping some bits to flip to 0
            indices_to_flip = rng.choice(
                indices_occupied, size=round(n_diff_left[i]), replace=False, p=p_choice
            )
            bitstring_matrix[i, :partition_size][indices_to_flip] = False

        # too few electrons in LEFT bits
        if n_diff_left[i] < 0:
            indices_empty = np.where(np.logical_not(bitstring_matrix_left[i]))[0]
            # Get the probabilities that each 0 should be flipped to 1
            p_choice = probs_left[i, np.logical_not(bitstring_matrix_left[i])] / np.sum(
                probs_left[i, np.logical_not(bitstring_matrix_left[i])]
            )
            # Correct the hamming by probabilistically flipping some bits to flip to 1
            indices_to_flip = rng.choice(
                indices_empty, size=round(np.abs(n_diff_left[i])), replace=False, p=p_choice
            )
            bitstring_matrix[i, :partition_size][indices_to_flip] = np.logical_not(
                bitstring_matrix_left[i][indices_to_flip]
            )
        ...
        # too many electrons in RIGHT bits
        if n_diff_right[i] > 0:
            indices_occupied = np.where(bitstring_matrix_right[i])[0]
            # Get the probabilities that each 1 should be flipped to 0
            p_choice = probs_right[i, bitstring_matrix_right[i]] / np.sum(
                probs_right[i, bitstring_matrix_right[i]]
            )
            # Correct the hamming by probabilistically flipping some bits to flip to 0
            indices_to_flip = rng.choice(
                indices_occupied, size=round(n_diff_right[i]), replace=False, p=p_choice
            )
            bitstring_matrix[i, partition_size:][indices_to_flip] = np.logical_not(
                bitstring_matrix_right[i][indices_to_flip]
            )

        # too few electrons in RIGHT bits
        if n_diff_right[i] < 0:
            indices_empty = np.where(np.logical_not(bitstring_matrix_right[i]))[0]
            # Get the probabilities that each 1 should be flipped to 0
            p_choice = probs_right[i, np.logical_not(bitstring_matrix_right[i])] / np.sum(
                probs_right[i, np.logical_not(bitstring_matrix_right[i])]
            )
            # Correct the hamming by probabilistically flipping some bits to flip to 1
            indices_to_flip = rng.choice(
                indices_empty, size=round(np.abs(n_diff_right[i])), replace=False, p=p_choice
            )
            bitstring_matrix[i, partition_size:][indices_to_flip] = np.logical_not(
                bitstring_matrix_right[i][indices_to_flip]
            )
