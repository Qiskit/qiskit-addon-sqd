...
This package contains the Qiskit addon for sample-based quantum diagonalization (SQD) --- a technique for finding eigenvalues and eigenvectors of quantum operators, such as a quantum system Hamiltonian, using quantum and distributed classical computing together `[1–5] <references_>`_. This technique can be run on current quantum computers and has been shown to scale to problem sizes beyond what was possible with variational methods --- and even beyond the reach of exact classical diagonalization methods.
