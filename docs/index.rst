...
``qiskit-addon-sqd`` can be used to classically process samples drawn from a variety of quantum circuits in practice. For example:
    ...
    3. A set of Krylov basis states implemented with qDRIFT randomized compilation of the time evolution operator. This approach yields shorter-depth circuits compared to Trotter-based decompositions of the time evolution and so can be used for quantum chemistry Hamiltonians. This technique has been applied to the ground state energy of polycyclic aromatic hydrocarbons `[6] <ref6_>`_.
